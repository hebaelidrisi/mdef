...
=== "Cognitive Orgies III"
    ...
    *image of dark enclosure reference
    ...
    ### **Process, Evolution & Traces**
    - [Hackster.io Repository](https://www.hackster.io/549006/sensescape-5b950e)*
    ...
    *insert image of hexagons on Rhino
    ...
    1. The Physical Artifact - The first mental challenge came when we went back to the idea of having the material on each module being concealed so its not seen, meaning the units would need to be three-dimensional or have a depth so materials could be placed inside and not be visible, only accessible through touch. There was a lot of back and forth about whether to make independent modules, wrapped in aluminium foil from the inside with the sensors and electronics inside and only a hole from the top or the side for users to access the material, or to make one unit. Figuring out how to construct multiple hexagon boxes, that would be sturdy enough for people to insert their hands and feet which were also spread across the space seemed very tedious. After discussing amongst ourselves and with the Fab Lab Team, we realized the smarter route to take would be to build one unit with the hexagons as insertion points, channeling the same physical structure as Twister, but with three dimensionality. The construction of the unit involved a combination of tools, including laser cutting, woodworking and manual production. 
    2. Sensors - when we first tried to connect one unit to the sensor and get the readings in TouchDesigner to start playing with the sound outputs, it was working just fine. When we then tried to connect more than one sensor at the same time, it was no longer giving normal readings - the values were extremely high and we realized there was something in the code which was mapping the values. After troubleshooting that part, the sensor then wasn’t reading anything when it was touched alone. With Mikel’s help we realized that the circuit needed a ground, so we had to either touch the computer or the barduino board to ground it, which was a really interesting issue. I never knew the sensors needed to be ‘grounded’ to give correct readings. I found this to be a fascinating discovery that could relate to my personal design project and it sparked my curiosity to learn more about this. 
    3. Program - one thing we have learnt about TouchDesigner over the course of this program is that it can be notoriously unreliable when it comes to sound outputs. Many times you set something up and it works normally when you are testing it and then when you try to test it another time, it gives a different output or doesn’t work, and it has nothing to do with the sensors or readings its getting. Even though before the final presentations we tested all the sensors and sounds and everything was working fine, right before our time to present, the sound outputs were not working properly. Only one module would generate a sound, or three modules would generate the same sound, which was ruining our intention of each module generating a sound that reflected the nature of the material, for the viewers who cannot see the materials to understand what kind of material was being touched. Unfortunately, we could not resolve the problem for the presentation, but we proceeded with the live performance. Afterwards, Beste did some research and figured out that the issue had something to do with the specific node being used to combine the inputs into one output - we were using ‘Combine Channels’ while we should have been using ‘Combine CHOPS’. Additionally, we needed to add one Math CHOP before each sensor signal to resolve the issue of some modules not producing any sound. 
